## List of currently implemented (and planned) Freecord's features

Because Freecord is based on Discord Website and (unlike official client and
...
However, Freecord client is capable of doing some tweaks to it, implementing
...
| Translations | Freecord's localization support for multiple languages. | ✅️ Done |
| Hiding the Discord's side bar | Switching the visibility state of side bar containing channel list. This makes Freecord working on the devices with the smaller screens, like e.g. PinePhone smartphones. | ✅️ Done |
| Node.js based Freecord extensions support | Freecord-specific add-on implementation that allows for using Node dependencies. | ❌️ **TODO** |
...
| Freecord update notifications | Notifies the user whenever new Freecord version is out | ✅️ Done |
...
| Custom Discord and Freecord styles | Allow styling Discord and Freecord with own themes. | 1️⃣ ***Alpha*** (Only Discord can be styled) |
...
| Connect through proxy | Feature to connect to app with proxies and break filtering | 🚧️ ***Under construction***
...
Currently Freecord loads CSS themes from `{userData}/Themes/` directory when
