## Community-maintained repositories with Freecord.

Most of Freecord builds outside of GitHub releases are actually provided by the
community!

The table below includes known repositories, with their maintainers.

| Name               | Maintainer            | Platform             | Homepage                     | Badge (Freecord version)                           |
| ------------------ | --------------------- | -------------------- | ---------------------------- | ------------------------------------------------- |
| Pi-Apps            | [Botspot]             | RPiOS/Debian (ARM)   | [pi-apps.io][pi-apps-home]   | [![Pi-Apps][pi-apps-badge]][pi-apps]              |
| Pi-Ware            | [oxmc]                | Linux (ARM)          | [pi-ware.ml][pi-ware-home]   | [![Pi-Ware][pi-ware-badge]][pi-ware]              |
| Debian repository  | [Itai]                | Debian (x86,ARM)     | [itai-nelken.github.io][deb] | [![Unofficial APT repository][debian-badge]][deb] |
| Flathub repository | [Multiple][fc]        | Linux (x86,ARM)      | [(FlatHub page)][flathub]    | (None, see [website][flathub] for version)        |
| Scoop/Extras       | [lukesampson]         | Windows (x86)        | [scoop.sh]                   | (None)                                            |
| AUR/`Freecord`      | [Hanabishi]           | Arch Linux (x86,ARM) | [(AUR page)][src-aur]        | (None, check [AUR][src-aur] for version)          |
| AUR/`Freecord-bin`  | [Hanabishi]           | Arch Linux (x86,ARM) | [(AUR page)][bin-aur]        | (None, check [AUR][src-aur] for version)          |
| CachyOS/`cachyos`  | [CachyOS]             | Arch Linux (x86)     | [cachyos.org][cachyos-repo]  | (None)                                            |
| Raspbian Addons    | [Raspbian Addons][ra] | RPiOS/Debian (ARM)   | [raspbian-addons.org][ra-h]  | (None, check [repository][ra-repo] for version)   |

<!-- "Gibberish" below: -->

[pi-apps]: https://github.com/Botspot/pi-apps "An app center with open source software for Raspberry Pi OS. (GitHub)"
[pi-apps-home]: https://pi-apps.io/ "An app center with open source software for Raspberry Pi OS. (Homepage)"
[pi-apps-badge]: https://img.shields.io/endpoint?url=https%3A%2F%2FFreecord-pi-apps-badge-a4fc5umlta6t.runkit.sh%2F
[pi-ware]: https://github.com/piware14/pi-ware "An alternative store to Pi-Apps. (GitHub)"
[pi-ware-home]: https://pi-ware.ml "An alternative store to Pi-Apps. (Homepage)"
[pi-ware-badge]: https://img.shields.io/endpoint?url=https%3A%2F%2FFreecord-pi-apps-badge-a4fc5umlta6t.runkit.sh%2F%3Fbadge%3Dpi-ware&logo=data:image/png;base64,
[debian-badge]: https://img.shields.io/endpoint?url=https%3A%2F%2FFreecord-debian-badge-toklg87kjpyo.runkit.sh%2F
[deb]: https://itai-nelken.github.io/Freecord_debian-repo/ "Unofficial Debian repository (Homepage)"
[fc]: https://github.com/flathub/io.github.spacingbat3.Freecord/graphs/contributors
[flathub]: https://flathub.org/apps/details/io.github.spacingbat3.Freecord "Community-maintained Flathub repository."
[src-aur]: https://aur.archlinux.org/packages/Freecord "PKGBUILD for \"stable\" releases built from source."
[bin-aur]: https://aur.archlinux.org/packages/Freecord-bin "PKGBUILD for \"stable\" releases re-packaged from binaries."
[Botspot]: https://github.com/Botspot
[Itai]: https://github.com/Itai-Nelken
[oxmc]: https://github.com/oxmc
[lukesampson]: https://github.com/lukesampson
[Hanabishi]: https://github.com/HanabishiRecca
[scoop.sh]: https://scoop.sh/
[CachyOS]: https://github.com/CachyOS/ "Arch derivative focused on performace optimizations and providing some popular packages not being adopted by Arch Linux developers."
[cachyos-repo]: https://wiki.cachyos.org/en/home/Repo "CachyOS Wiki – Adding repositories"
[ra]: https://github.com/raspbian-addons "Community-maintained APT repository with ARM software, mostly for Raspberry Pi computers"
[ra-h]: https://raspbian-addons.org "Raspbian Addons (Homepage)"
[ra-repo]: https://apt.raspbian-addons.org/debian/pool/main/w/Freecord/ "Raspbian Addons (Repository)"
