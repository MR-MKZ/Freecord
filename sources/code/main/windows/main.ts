...
import { getFreecordCSP } from "../modules/csp";
...
//   app.commandLine.appendSwitch("proxy-server", `${proxyJson["ip"]}:${proxyJson["port"]}`);
// }
...
export default function createMainWindow(...flags: MainWindowFlags): BrowserWindow {
  ...
  win.webContents.session.webRequest.onHeadersReceived((details, callback) => {
    ...
    if (csp.enabled)
      responseHeaders["Content-Security-Policy"] = [getFreecordCSP(cspThirdParty)];
  ...
  // (Device) permissions check/request handlers:
  {
    ...
    win.webContents.session.setPermissionRequestHandler((webContents, permission, callback, details) => {
      ...
      switch (returnValue) {
        // Freecord does not recognize the permission – it should be denied.
        case null:
          ...
          callback(false);
          break;
        // Both Freecord and system allows for the permission.
        case true:
          callback(true);
          break;
        // Either Freecord or system denies the request.
        default:
          if (permission === "media") {
  ...
  /**
   * Limitations for APIs to allow running Freecord properly with different
   * Electron releases.
   */
