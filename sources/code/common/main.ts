...
import { app, BrowserWindow, dialog, session, screen, ipcMain as ipc } from "electron/main";
...
import { initWindow } from "../main/modules/parent";
  ...
  console.debug = ((message:unknown,...optionalParams:unknown[]) => void import("@spacingbat3/kolor")
    ...
    .then(kolor => {
      if((__filename.startsWith(app.getAppPath()) ?
        debug("Freecord").enabled : false) || argv.values.verbose === true)
...
let startHidden = false,
  /** Whenever Freecord starts in "Safe Mode". Default is `false`. */
  safeMode = false;
...

{
  ...
  if(argv.values.help === true || argv.values.info === true) {
    ...
    console.log([
      ...
      renderLine("add-css-theme", "Adds theme to Freecord using file picker."),
      renderLine("safe-mode", "Starts Freecord in 'Safe Mode'.")/*,
      renderLine(["remove-css-theme=" + kolor.yellow("{name}")], "Removes Freecord theme by "+kolor.yellow("{name}")),
      renderLine(["list-css-themes"], "Lists currently added Freecord themes")*/
    ].sort().join("\n")+"\n");
    app.exit();
  }
...
}
{
  ...
  // Workaround #236: Freecord calls appear as players in playerctl
...
function main(): void {
  if (overwriteMain) {
    ...
    const proxy = initWindow("proxy", mainWindow, {
      width: 600,
      height: 480,
      resizable: false,
    ...
    if (proxy) {
      proxy.once("ready-to-show", () => {
        const proxyData = readFileSync(path.join(__dirname, "..", "utility", "proxy.json"), "utf-8");
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        const proxyJson = JSON.parse(proxyData);
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        if (proxyJson["ip"] === null && proxyJson["port"] === null) {
          proxy.show();
        }
      });
      ...
      ipc.on("proxy.connect", (event, ip: string, port: string) => {
        if(!proxy.isDestroyed() && event.senderFrame.url === proxy.webContents.getURL()) {
          const data = {
            "ip": ip,
            "port": port
          };

          writeFileSync(path.join(__dirname, "..", "utility", "proxy.json"), JSON.stringify(data, null, 2));
          dialog.showMessageBox({
            message: "You have to restart app for connecting to proxy!",
            type: "info",
            title: "App need restart"
          }).then(() => {
            mainWindow.close();
          }).catch((err: unknown) => {
            console.error(err);
