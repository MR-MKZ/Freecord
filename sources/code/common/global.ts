...
export function wLog(msg: string): void {
  console.log("%c[Freecord]", "color: #69A9C1", msg);
}
...
export const knownInstancesList = Object.freeze([
  ...
  ["Discord Canary",           new URL("https://canary.discord.com/app"),    false ],
  ["Discord Public Test Beta", new URL("https://ptb.discord.com/app"),       false ]
] as const) satisfies readonly (readonly [ name: string, url: URL, active: boolean ])[];

/**
 * An object which type includes information about the Freecord's build
...
export interface BuildInfo {
   ...
   *  in `PascalCase`). Defaults to `SpacingBat3.Freecord`. For forks and
   * community builds, the recommended value of `AppUserModelId` is
   * `{AuthorOrPackager}.Freecord` or `{Author}.{Name}`.
   * 
   * @platform win32
   */
  ...
  features: {
    /**
     * Used to disable notifications on new Freecord releases. It does not
     ...
     * way of checking updates and/or updating the Freecord. This mostly applies
     ...
     * <https://github.com/Mr-MKZ/Freecord.git>).
     */
