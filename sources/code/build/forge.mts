...
const appUserModelId = process.env["FREECORD_WIN32_APPID"];
const flatpakId = process.env["FREECORD_FLATPAK_ID"]?.toLowerCase() ??
  "io.github.mrmkz.freecord";
const author = packageJson.data.author !== undefined ? new Person(packageJson.data.author).name : "Mr.MKZ";
...
const env = {
  asar: process.env["FREECORD_ASAR"]?.toLowerCase() !== "false",
  build: process.env["FREECORD_BUILD"]?.toLowerCase()
};
...
const config:ForgeConfig = {
  ...
  makers: [
    ...
    new MakerSquirrel((arch) => ({
      ...
      fixUpPaths: true,
      iconUrl: `https://raw.githubusercontent.com/Mr-MKZ/Freecord/${packageJson.data.version}/${iconFile}.ico`,
      noDelta: true
    })),
    ...
    ...(process.env["Freecord_ALL_MAKERS"]?.toLowerCase() === "true" ? [
      new MakerWix({
        appUserModelId: appUserModelId ?? author+".Freecord",
        ...
        shortName: "Freecord",
        programFilesFolderName: "Freecord",
        shortcutFolderName: "Freecord"
      }),
      /*
    ...
    packageAfterCopy: async (_forgeConfig, path, _electronVersion, platform) => {
      ...
      async function writeBuildInfo() {
        const buildConfig: BuildInfo = {
          ...
          features: {
            updateNotifications: process.env["FREECORD_UPDATE_NOTIFICATIONS"] !== "false"
          }
        };
